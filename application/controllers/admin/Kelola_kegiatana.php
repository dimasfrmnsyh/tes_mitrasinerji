...
class Kelola_kegiatan extends CI_Controller {
    ...
    public function tambah() {
        if($this->input->post('submit')) { 
            ...
            if ($this->form_validation->run() == false) {
                ...
                $this->load->view('admin/kelola_kegiatan/tambah');
            } else {

                $data['tanggal'] = $this->input->post('tanggal'); 
                $data['jam_mulai'] = $this->input->post('jam_mulai'); 
                $data['jam_selesai'] = $this->input->post('jam_selesai'); 
                $data['uraian_kegiatan'] = $this->input->post('uraian_kegiatan'); 
                $data['tempat_kegiatan'] = $this->input->post('tempat_kegiatan'); 
                $data['alamat'] = $this->input->post('alamat'); 
                $data['pelaksana'] = $this->input->post('pelaksana'); 
                $data['keterangan'] = $this->input->post('keterangan'); 

                if($this->model_kegiatan->insert($data)) {
    ...
    public function edit($id_kegiatan = 0) {
        ...
        if($this->input->post('submit')) {
                ...
                $data_edit['pelaksana'] = $this->input->post('pelaksana'); 
            ...
            if($data['kegiatan']->pelaksana != $data_edit['pelaksana']) {
            $this->form_validation->set_rules('pelaksana', 'Pelaksana', 'trim|required'); 
            }  
    ...
    public function print(){
        ...
        $sheet->setCellValue('C2' , 'jam_mulai'); 
        $sheet->setCellValue('D2' , 'jam_selesai'); 
        $sheet->setCellValue('E2' , 'uraian_kegiatan'); 
        $sheet->setCellValue('F2' , 'tempat_kegiatan'); 
        $sheet->setCellValue('G2' , 'alamat'); 
        $sheet->setCellValue('H2' , 'pelaksana'); 
        ...
        foreach ($data as $idx => $val) {  
           ...
           $sheet->setCellValue('H' . ($idx+3), $val->pelaksana); 
           $sheet->setCellValue('I' . ($idx+3), $val->keterangan); 
